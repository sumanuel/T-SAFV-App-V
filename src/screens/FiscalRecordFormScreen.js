...
function getUnitLabel(unit) {
  return (
    [unit?.marca, unit?.modelo, unit?.ano].filter(Boolean).join(" ") ||
    "Unidad sin descripción"
  );
}

export default function FiscalRecordFormScreen({
  initialUnit,
  onBack,
  onSaved,
}) {
  ...
  const [chofer, setChofer] = useState(
    initialUnit?.ultimo_chofer || initialUnit?.chofer || "",
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setUnits(data);
        setSelectedUnitId(
          (current) => current || initialUnit?.id || data[0]?.id || null,
        );
      } catch {
        Alert.alert(
          "Fiscalización",
          "No se pudieron cargar las unidades activas.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async () => {
    if (!asociacionId || !selectedUnitId) {
      Alert.alert(
        "Datos incompletos",
        "Selecciona una unidad para registrar la fiscalización.",
      );
      return;
    }
    ...
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        Alert.alert(
          "Fiscalización registrada",
          "La traza operativa fue guardada correctamente.",
        );
        onSaved?.();
      } else {
        Alert.alert(
          ...
          res.data?.message ||
            res.data?.error ||
            "No se pudo guardar la fiscalización.",
        );
      }
    } catch (error) {
      Alert.alert(
        "Error",
        error?.message || "No se pudo guardar la fiscalización.",
      );
    } finally {
      setSubmitting(false);
    }
        ...
        <ScrollView contentContainerStyle={styles.container}>
          ...
          >
            ...
            {loading ? (
              ...
              <Text style={[styles.helpText, { color: colors.textSecondary }]}>
                No hay unidades activas para fiscalizar.
              </Text>
            )}
            ...
            {selectedUnit ? (
              ...
              >
                <Text style={[styles.unitSummaryTitle, { color: colors.text }]}>
                  {getUnitLabel(selectedUnit)}
                </Text>
                <Text
                  style={[styles.unitSummaryAccent, { color: colors.accent }]}
                >
                  Placa: {selectedUnit.placa || "Sin placa"}
                </Text>
                <Text
                  style={[styles.unitSummaryCode, { color: colors.primary }]}
                >
                  Unidad N° {selectedUnit.numero_unidad || selectedUnit.id}
                </Text>
                <Text
                  style={[
                    styles.unitSummaryMeta,
                    { color: colors.textSecondary },
                  ]}
                >
                  Propietario:{" "}
                  {[
                    selectedUnit.propietario_nombre,
                    selectedUnit.propietario_apellido,
                  ]
                    .filter(Boolean)
          ...
          >
            ...
            <Field
              ...
              value={pasajeros}
              onChangeText={(value) =>
                setPasajeros(value.replace(/[^0-9]/g, ""))
              }
              placeholder="0"
